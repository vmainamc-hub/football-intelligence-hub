    ...
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          ...
          const fixtures = groups.flatMap((group) => group.matches.filter((match) => match.hg === undefined && match.ag === undefined));
          const sourceCounts = fixtures.reduce<Record<string, number>>((acc, fixture) => {
            const source = fixture.source ?? "free";
