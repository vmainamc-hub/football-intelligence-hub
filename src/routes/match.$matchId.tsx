...
function MatchIntelligence() {
  ...
  const bestAction = marketMap.filter((m) => m.market !== "1X2").sort((a, b) => b.probability - a.probability)[0] ?? oneX2;
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-6xl px-5 py-6 lg:px-10">
        ...
        <section className="mt-6 panel p-5"><div className="flex items-center gap-2"><Database className="size-4 text-primary" /><div className="label-xs text-primary">EVIDENCE LINEAGE</div></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[["Source",result.pipeline.source],["Competition",result.pipeline.competition],["Historical rows",String(result.pipeline.historicalRowsLoaded)],["Home sample",String(result.pipeline.homeSample)],["Away sample",String(result.pipeline.awaySample)],["H2H sample",String(result.pipeline.h2hSample)],["Evidence mode",result.pipeline.evidenceMode],["Generated",result.pipeline.generatedAt]].map(([k,v]) => <div key={k} className="rounded border border-border bg-card p-3"><div className="label-xs">{k}</div><div className="mt-1 break-all text-xs font-medium">{v}</div></div>)}</div></section>
      </main>
    </div>
  );
}
