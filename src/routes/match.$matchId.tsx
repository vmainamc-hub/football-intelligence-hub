...
function MatchIntelligence() {
  ...
  const living = (result.aiReasoningPacket as Record<string, unknown> | undefined)
    ?.livingEvidence as Record<string, unknown> | undefined;
      ...
      <main className="mx-auto max-w-6xl px-5 py-6 lg:px-10">
        ...
        <section className="mt-6 panel p-6">
          ...
          <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
            All seven requested alternatives are scored independently from the same accumulated
            historical evidence and authoritative engine ensemble. The display does not collapse the
            comparison to a single preferred market.
          </p>
          <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {sevenMarkets.map((m) => (
              <MarketCard key={`${m.market}-${m.selection}`} market={m} />
            ))}
          </div>
        </section>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {livingFamilies.length ? (
              ...
              <span className="text-xs text-muted-foreground">
                No living-cell source families have been persisted for this fixture yet.
              </span>
            )}
          </div>
        </section>
        ...
        <section className="mt-6 panel p-6">
          ...
          <p className="mt-4 text-xs text-muted-foreground border-t border-border pt-3">
            * Explanations are generated deterministically from observed engine evidence, historical
            samples, and mathematical consensus. No external generative AI model overrides the
