...
export function ModulePage({
  ...
  eyebrow: string;
  description: string;
  stages: { name: string; detail: string; live?: boolean }[];
}) {
  return (
    <div className="mx-auto max-w-6xl px-5 py-10 lg:px-10">
      <Link to="/" className="label-xs inline-flex items-center gap-2 hover:text-foreground">
      ...
      <div className="mt-7 grid gap-3">
        {stages.map((s) => (
          <div key={s.name} className="panel flex items-start gap-4 p-5">
            ...
            <span className="label-xs">{s.live ? "ACTIVE" : "READY"}</span>
          </div>
        ))}
      </div>
    </div>
