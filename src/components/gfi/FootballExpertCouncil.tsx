    ...
    <section className="mt-6 panel border-primary/40 p-6 shadow-sm">
      ...
      {call && (
        <div className="mt-6 rounded-xl border-2 border-primary/40 bg-primary/5 p-5">
          <div className="label-xs text-primary">FINAL AI ANALYST CALL</div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded border border-border bg-card p-3"><div className="label-xs">IDENTITY CHECK</div><div className={`mt-1 font-semibold ${tone(panel.identityCheck.status)}`}>{panel.identityCheck.status}</div><div className="mt-2 text-[11px] text-muted-foreground">Home {panel.identityCheck.homeConfidence}% · Away {panel.identityCheck.awayConfidence}% · Competition {panel.identityCheck.competitionConfidence}%</div></div>
        <div className="rounded border border-border bg-card p-3"><div className="label-xs">FOOTBALL REALITY</div><div className={`mt-1 font-semibold ${tone(panel.realityCheck.status)}`}>{panel.realityCheck.status}</div><div className="mt-2 text-[11px] text-muted-foreground">Reality score {panel.realityCheck.score}/100</div></div>
        <div className="rounded border border-border bg-card p-3"><div className="label-xs">CHAIR DECISION</div><div className={`mt-1 font-semibold ${tone(panel.chair.decision)}`}>{panel.chair.decision}</div><div className="mt-2 text-[11px] text-muted-foreground">Severity {panel.chair.severity}</div></div>
        <div className="rounded border border-border bg-card p-3"><div className="label-xs">COUNCIL MARKET</div><div className="mt-1 font-semibold">{panel.marketReview.selectedMarket || "Not supplied"}</div><div className="mt-2 text-[11px] text-muted-foreground">Decision-layer selection</div></div>
      </div>
