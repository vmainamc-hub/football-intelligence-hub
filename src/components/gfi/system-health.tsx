...
import { AlertTriangle, CheckCircle2, Database, RefreshCw, Globe2 } from "lucide-react";
...
export function SystemHealth() {
  ...
  const harvest = useMutation({
    mutationFn: async () => {
      const result = await harvestOpenFootballUniverse({ data: { limit: 120 } });
    ...
    <div className="mx-auto max-w-7xl px-5 py-8 lg:px-10">
      ...
      <header className="mt-7 border-b border-border pb-6">
        ...
        <p className="mt-2 max-w-3xl text-sm text-muted-foreground">
          ...
          The global discovery fabric can enumerate public OpenFootball datasets and ingest them
          into the reservoir.
        </p>
      </header>
      <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_420px]">
          ...
          {row && (
            <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {[
                ...
                [
                  "SIMULATION",
                  row.analysis.pipeline.simulation.iterations.toLocaleString() + " runs",
                ],
        ...
        <div className="panel p-5">
          <div className="flex items-center gap-2">
            ...
            <div className="label-xs">PERSISTENT RESERVOIR</div>
          </div>
          {reservoir.isLoading && (
            <div className="mt-4 text-xs text-muted-foreground">
              Checking cloud football memory…
            </div>
          )}
          {reservoir.data && !reservoir.data.configured && (
      ...
      <div className="panel mt-5 p-5">
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            disabled={harvest.isPending || !discovery.data?.length}
        ...
        {harvest.data && (
          <div className="mt-4 text-xs text-muted-foreground">
            Harvest completed: {harvest.data.inserted.toLocaleString()} rows written/updated in the
            reservoir.
          </div>
        )}
        {harvest.error && (
      ...
      <div className="panel mt-5 p-5">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          ...
          graph. Searches combine persistent reservoir history with live source discovery. Advanced
          engines run on the same historical evidence package.
        </p>
      </div>
    </div>
  );
