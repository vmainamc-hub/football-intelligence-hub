...
import {
  runLivingBatchAnalysis,
  type BatchAnalysisResponse,
  type BatchSelection,
} from "@/lib/gfi/batch-analysis-living";
...
export function BatchLab() {
  ...
  const submit = () => {
    ...
    if (value && !mutation.isPending) mutation.mutate(value);
  };
  return (
    <Shell>
      <section className="panel p-5">
        ...
        <h2 className="mt-2 text-xl font-semibold">
          Build a batch from the living fixture universe
        </h2>
        <p className="mt-2 text-sm text-muted-foreground">
          Each batch warms reusable team and match evidence cells, overscans to prevent duplicate
          selections, and then applies the same authoritative engine used for single-match analysis.
        </p>
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_390px]">
            <div className="panel overflow-hidden">
              ...
              {result.selections.map((row, i) => (
                <button
                  key={`${row.fixture.date}-${row.fixture.home}-${row.fixture.away}-${i}`}
  ...
  return (
    <>
      ...
      <div className="mt-3 text-xs text-muted-foreground">
        {selected.analysis.pipeline.historicalRowsLoaded} evidence rows assembled ·{" "}
        {selected.analysis.pipeline.homeRowsMatched} home-team rows used ·{" "}
        {selected.analysis.pipeline.awayRowsMatched} away-team rows used ·{" "}
        {selected.analysis.pipeline.h2hSample} H2H matches
      </div>
      <button
    ...
    <div className="mx-auto max-w-7xl px-5 py-8 lg:px-10">
      ...
      <header className="mt-7 border-b border-border pb-6">
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
          Living reservoir-backed batch prediction with canonical fixture identity and evidence
          warming.
        </p>
      </header>
