...
export const discoverOpenFootballUniverse = createServerFn({ method: "GET" }).handler(
  async (): Promise<Source[]> => {
    const repos = (await getJson(
      `${GH}/orgs/openfootball/repos?per_page=100&sort=updated`,
    )) as any[];
    const out: Source[] = [];
    for (const repo of repos) {
      ...
      for (const f of (tree?.tree ?? [])
        .filter((x: any) => x.type === "blob" && /\.json$/i.test(String(x.path ?? "")))
        .slice(0, 200)) {
        out.push({
          name: `openfootball/${name}:${f.path}`,
          url: `${RAW}/${name}/${repo.default_branch ?? "master"}/${f.path}`,
          region: name,
          priority: name === "world" ? 1 : 2,
        });
      }
    }
    return out.sort((a, b) => a.priority - b.priority).slice(0, 5000);
  },
);
          ...
          Number.isFinite(hg) && Number.isFinite(ag)
            ? hg! > ag!
              ...
              : hg === ag
                ...
                : "A"
            : undefined,
        league: String(payload?.name ?? source),
        source: "openfootball",
        sourceId: `${source}#${i}`,
      },
    ];
...
export const harvestOpenFootballUniverse = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const sources = await discoverOpenFootballUniverse(),
      sample = sources.slice(0, Math.max(10, Math.min(data.limit ?? 120, sources.length))),
      rows: MatchRow[] = [];
    for (const s of sample) {
      try {
        const r = await fetch(s.url, { headers: { Accept: "application/json" } });
        if (r.ok) rows.push(...parse(await r.json(), s.name));
      } catch {
