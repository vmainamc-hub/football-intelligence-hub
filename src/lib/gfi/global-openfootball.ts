...
const EXCLUDED = new Set(["awesome-football", "openfootball.github.io", "quick-starter", "sandbox", "spec", "v0-format", "opendata-theme"]);
...
async function discoverInternal(): Promise<Source[]> {
  if (discoveryCache && Date.now() - discoveryCache.at < DISCOVERY_TTL_MS) return discoveryCache.sources;
  ...
  for (const repo of repos) {
    ...
    const tree = await getJson(`${GH}/repos/openfootball/${name}/git/trees/${repo.default_branch ?? "master"}?recursive=1`).catch(() => null);
    for (const f of (tree?.tree ?? []).filter((x: any) => x.type === "blob" && /\.(json|txt)$/i.test(String(x.path ?? ""))).slice(0, 500)) {
      const path = String(f.path ?? "");
      if (!path) continue;
      out.push({ name: `openfootball/${name}:${path}`, url: `${RAW}/${name}/${repo.default_branch ?? "master"}/${path}`, region: name, priority: name === "world" ? 1 : 2 });
    }
  }
...
export const discoverOpenFootballUniverse = createServerFn({ method: "GET" }).handler(async (): Promise<Source[]> => discoverInternal());

function parse(payload: any, source: string): MatchRow[] {
  const arr = Array.isArray(payload?.matches) ? payload.matches : Array.isArray(payload) ? payload : [];
  return arr.flatMap((x: any, i: number) => {
    const date = String(x?.date ?? "").slice(0, 10), home = String(x?.team1 ?? x?.home ?? "").trim(), away = String(x?.team2 ?? x?.away ?? "").trim();
    if (!date || !home || !away) return [];
    const ft = Array.isArray(x?.score?.ft) ? x.score.ft : [], hg = ft[0] == null ? undefined : Number(ft[0]), ag = ft[1] == null ? undefined : Number(ft[1]);
    return [{ date, time: typeof x?.time === "string" ? x.time.slice(0, 5) : undefined, home, away, hg: Number.isFinite(hg) ? hg : undefined, ag: Number.isFinite(ag) ? ag : undefined, result: Number.isFinite(hg) && Number.isFinite(ag) ? hg! > ag! ? "H" : hg === ag ? "D" : "A" : undefined, league: String(payload?.name ?? source), source: "openfootball", sourceId: `${source}#${i}` }];
  });
}
...
async function fetchSource(source: Source) {
  try {
    ...
    try { return parse(JSON.parse(text), source.name); } catch { return []; }
  } catch { return []; }
}
