...
function actionableScore(analysis: ServerMatchAnalysis) {
  ...
  return (
    ...
    analysis.consensus.agreement * 15 -
    analysis.consensus.conflict * 20
  );
}
...
export const runBatchAnalysis = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<BatchAnalysisRow[]> => {
    ...
    const rows = groups.flatMap((group) =>
      group.matches
        .filter((m) => (includeUpcoming ? true : m.hg !== undefined && m.ag !== undefined))
        .map((m) => ({ ...m, league: group.league, code: group.code, season: group.season })),
    );
    const unique = new Map<string, MatchRow & { league: string; code: string; season: string }>();
    for (const row of rows)
      unique.set(
        `${dateKey(row.date)}|${row.home}|${row.away}|${row.time ?? ""}|${row.source ?? ""}`,
        row,
      );
    const ordered = [...unique.values()].sort((a, b) =>
      kickoffValue(a).localeCompare(kickoffValue(b)),
    );
    const target = includeUpcoming
      ? ordered
          .filter((m) => m.hg === undefined || m.ag === undefined)
          .slice(0, Math.max(1, Math.min(data.limit ?? 20, 40)))
      : ordered.slice(-Math.max(1, Math.min(data.limit ?? 20, 40)));
