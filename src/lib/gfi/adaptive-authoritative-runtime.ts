...
export function analyzeAdaptiveAuthoritatively(fixture: MatchRow, rows: MatchRow[]): AuthoritativeMatchAnalysis {
  ...
  const over = (line: number) => { let sum = 0; for (let k = 0; k <= Math.floor(line); k++) sum += poisson(totalMean, k); return clamp(1 - sum); };
  const bttsYes = clamp((1 - Math.exp(-homeLambda)) * (1 - Math.exp(-awayLambda)));
  const adaptiveEngine: EngineOutput = {
    id: "MOMENTUM", name: "Adaptive Recency + Regime", version: "arrm-v1",
    ...
    quality: Math.round(clamp(0.48 + Math.min(1, (home.played + away.played) / 24) * 0.38) * 100), probabilities: p,
    values: {
      lambdaHome: homeLambda, lambdaAway: awayLambda, expectedGoals: totalMean,
      homePPG: home.points / Math.max(1, home.played), awayPPG: away.points / Math.max(1, away.played),
      homeEffectiveSample: home.adaptive?.effectiveSample ?? home.played, awayEffectiveSample: away.adaptive?.effectiveSample ?? away.played,
      homeRegimeShift: home.adaptive?.regimeShiftScore ?? 0, awayRegimeShift: away.adaptive?.regimeShiftScore ?? 0,
      homeHalfLifeDays: home.adaptive?.halfLifeDays ?? 120, awayHalfLifeDays: away.adaptive?.halfLifeDays ?? 120,
      homeOpponentStrength: home.adaptive?.opponentStrength ?? 1, awayOpponentStrength: away.adaptive?.opponentStrength ?? 1,
    },
    evidence: [
    ...
    limitations: ["Adaptive regime is a quantitative recency layer, not an independent news or lineup source.", ...(Math.max(home.adaptive?.shrinkagePct ?? 0, away.adaptive?.shrinkagePct ?? 0) >= 35 ? ["Sparse samples are shrunk toward league/global priors."] : [])],
  };
  const engines = base.engines.filter((e) => e.id !== "MOMENTUM");
  engines.push(adaptiveEngine);
  const goals = engines.find((e) => e.id === "GOALS");
  if (goals) { goals.values = { ...goals.values, lambdaHome: homeLambda, lambdaAway: awayLambda, expectedGoals: totalMean, adaptiveRegime: 1 }; goals.version = `${goals.version}-arrm`; }
  const totals = engines.find((e) => e.id === "TOTALS");
  if (totals) { totals.values = { ...totals.values, expectedGoals: totalMean, "over0.5": over(0.5), "over1.5": over(1.5), "over2.5": over(2.5), "over3.5": over(3.5) }; totals.version = `${totals.version}-arrm`; }
  const btts = engines.find((e) => e.id === "BTTS");
  if (btts) { btts.values = { ...btts.values, yes: bttsYes, no: 1 - bttsYes }; btts.version = `${btts.version}-arrm`; }
  ...
  const predictedScore = (() => { let best = "1-1", bestP = 0; for (let h = 0; h <= 8; h++) for (let a = 0; a <= 8; a++) { const q = poisson(homeLambda, h) * poisson(awayLambda, a); if (q > bestP) { bestP = q; best = `${h}-${a}`; } } return best; })();
  const reasoning = { ...(base.aiReasoningPacket ?? {}), adaptiveRecencyRegime: adaptiveEngine.values, adaptivePolicy: "ARRM: recency decay + regime shift + current-season/venue weighting + opponent-strength adjustment + Bayesian-style shrinkage." };
  return {
    ...base, home, away, probabilities: p,
    totals: { ...base.totals, "over0.5": over(0.5), "over1.5": over(1.5), "over2.5": over(2.5), "over3.5": over(3.5) },
    btts: { yes: bttsYes, no: 1 - bttsYes }, engines, evidenceLedger,
    modelConvergence: Math.max(0, Math.min(1, Number(base.modelConvergence ?? 0) * 0.7 + (adaptiveEngine.confidence / 100) * 0.3)),
    predictedScore, predictedScoreState: (home.adaptive?.effectiveSample ?? 0) + (away.adaptive?.effectiveSample ?? 0) >= 8 ? "EVIDENCE_BACKED" : "PRIOR_BASED",
    ...
    finalPrediction: base.finalPrediction, aiReasoningPacket: reasoning, analysisVersion: "gfi-authoritative-v10-arrm",
