...
const toMatch = (r: ReservoirMatch): MatchRow => ({
  date: r.date,
  time: r.time,
  home: r.home,
  away: r.away,
...
const stripDate = (q: string) =>
  q
    .replace(/\b20\d{2}-\d{2}-\d{2}\b/g, " ")
    .replace(/\s+/g, " ")
    .trim();
const parseTeams = (q: string) => {
  const clean = stripDate(q),
    m = clean.match(/^(.+?)\s+(?:vs\.?|v\.?|versus|against)\s+(.+)$/i);
  return m ? { home: m[1].trim(), away: m[2].trim() } : { home: clean, away: "" };
};
const norm = (v: string) =>
  v
    .normalize("NFD")
