...
function matchKey(fixture: MatchRow) {
  return hash(
    `${dateKey(fixture.date)}|${canonicalTeamKey(fixture.home)}|${canonicalTeamKey(fixture.away)}`,
  );
}
...
function toMatchRow(value: unknown): MatchRow | null {
  ...
  if (typeof row.date !== "string" || typeof row.home !== "string" || typeof row.away !== "string")
    return null;
  const hg =
    typeof row.hg === "number"
      ? row.hg
      : Number.isFinite(Number(row.hg)) && row.hg !== "" && row.hg !== null
        ? Number(row.hg)
        : undefined;
  const ag =
    typeof row.ag === "number"
      ? row.ag
      : Number.isFinite(Number(row.ag)) && row.ag !== "" && row.ag !== null
        ? Number(row.ag)
        : undefined;
  return {
...
function summaryRows(
  summary: unknown,
  key: "historicalRows" | "recentRows" | "h2h" | "publicRows",
) {
...
export async function loadLivingEvidence(fixture: MatchRow): Promise<LivingEvidenceContext> {
  ...
  const [matchRes, homeRes, awayRes] = await Promise.all([
    db
      ...
      .select("summary,completeness,evidence_count,source_count,last_mined_at,status")
      .eq("match_key", matchKeyValue)
      .maybeSingle(),
    db
  ...
  const summary =
    matchSummary && typeof matchSummary === "object"
      ? (matchSummary as Record<string, unknown>)
      : {};
  const sourceFamilies = Array.isArray(summary.sourceFamilies)
    ? summary.sourceFamilies.filter((x): x is string => typeof x === "string")
    : [];
  const sourceFamilyCounts =
    summary.sourceFamilyCounts && typeof summary.sourceFamilyCounts === "object"
      ? (Object.fromEntries(
          Object.entries(summary.sourceFamilyCounts as Record<string, unknown>).filter(
            ([, value]) => typeof value === "number",
          ),
        ) as Record<string, number>)
      : {};
  const seen = new Set<string>();
  const unique = (rows: MatchRow[]) =>
    rows.filter((row) => {
      const id = rowKey(row);
  ...
  return {
    ...
    evidenceCount:
      Number(match?.evidence_count ?? 0) +
      Number(home?.evidence_count ?? 0) +
      Number(away?.evidence_count ?? 0),
    sourceCount: Math.max(
      Number(match?.source_count ?? 0),
      Number(home?.source_count ?? 0) + Number(away?.source_count ?? 0),
    ),
    lastMinedAt: [match?.last_mined_at, home?.last_mined_at, away?.last_mined_at]
      .filter(Boolean)
      .sort()
