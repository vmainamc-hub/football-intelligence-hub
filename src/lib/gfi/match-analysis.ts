...
export const analyzeFreeMatch = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<ServerMatchAnalysis> => {
    ...
    analysis.warnings = [...new Set([...analysis.warnings, `Research coverage ${research.coverage}% across ${research.distinctSources} source families.`])];
