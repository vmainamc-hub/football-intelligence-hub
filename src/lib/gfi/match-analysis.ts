...
function mergeMatches(rows: MatchRow[]) {
  ...
  return rows.filter(row => { const key=`${row.date}|${row.home.toLowerCase()}|${row.away.toLowerCase()}|${row.time??""}|${row.hg??""}|${row.ag??""}|${row.source??""}`; if(seen.has(key))return false; seen.add(key); return true; });
}
...
export const analyzeFreeMatch=createServerFn({method:"POST"}).validator((input:{code:string;fixture:MatchRow})=>input).handler(async({data}):Promise<ServerMatchAnalysis>=>{
  let [groups,research,living]=await Promise.all([
    loadFreeFixtures(),
    researchFixture({data:{query:`${data.fixture.home} vs ${data.fixture.away} ${data.fixture.date}`}}).catch(()=>({matches:[],reservoirMatches:0,liveMatches:0,distinctSources:0,sources:[],coverage:0,query:"",searchedAt:new Date().toISOString()})),
  ...
  const researchGroup:FreeLeague={league:data.fixture.league??targetGroup?.league??"Worldwide Football",code:`RESEARCH_${data.code||"GLOBAL"}`,season:"research",matches:researchRows,sourceUrl:"reservoir-plus-live-public-research",fetchedAt:research.searchedAt};
  ...
  analysis.pipeline.historicalRowsLoaded=Math.max(analysis.pipeline.historicalRowsLoaded,research.reservoirMatches+living.matchRows.length+living.homeTeamRows.length+living.awayTeamRows.length);
  analysis.pipeline.competitionsLoaded=expandedGroups.length;
  analysis.warnings=[...new Set([...analysis.warnings,`Public evidence ladder: ${research.coverage}% live/public coverage across ${research.distinctSources} live source families (${research.reservoirMatches} stored observations, ${research.liveMatches} live/public observations).`,`Living evidence cell: ${living.evidenceCount} accumulated observations, ${living.sourceCount} source records, ${living.sourceFamilies.length} distinct source families, ${living.matchCompleteness}% match completeness.`])];
  analysis.aiReasoningPacket={...analysis.aiReasoningPacket,research:{reservoirMatches:research.reservoirMatches,liveMatches:research.liveMatches,distinctSources:research.distinctSources,sources:research.sources,coverage:research.coverage,searchedAt:research.searchedAt},livingEvidence:{status:living.status,evidenceCount:living.evidenceCount,sourceCount:living.sourceCount,sourceFamilies:living.sourceFamilies,sourceFamilyCounts:living.sourceFamilyCounts,matchCompleteness:living.matchCompleteness,homeCompleteness:living.homeCompleteness,awayCompleteness:living.awayCompleteness,lastMinedAt:living.lastMinedAt,reusableHistoricalRows:living.matchRows.length+living.homeTeamRows.length+living.awayTeamRows.length,reusableH2HRows:living.h2hRows.length},researchPolicy:"ACCUMULATE_AND_REUSE_PUBLIC_EVIDENCE_BEFORE_TERMINAL_NO_DATA_STATE",onDemandMiningAttempted:directLivingRows===0};
