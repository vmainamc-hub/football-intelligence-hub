...
import { fetchUniversalFixturesInternal, UNIVERSAL_SOURCES } from "./universal-sources";
...
export const inspectSystem = createServerFn({ method: "GET" }).handler(async (): Promise<RuntimeSource[]> => {
  ...
  try {
    ...
    if (open) { open.observedRows = groups.reduce((n, g) => n + g.matches.filter((m) => m.source === "openfootball").length, 0); open.status = open.observedRows > 0 ? "CONNECTED" : "FAILED"; }
  } catch {
    const primary = out.find((s) => s.name === "Football-Data.co.uk");
  ...
  try {
    ...
    const rows = await fetchUniversalFixturesInternal({ dateFrom: today, dateTo: today });
    ...
    // Optional enrichment failures do not invalidate the primary data path.
  }
  return out;
});
