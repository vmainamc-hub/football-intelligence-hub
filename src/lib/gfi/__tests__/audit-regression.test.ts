...
test("audit regression: zero direct team evidence never creates the old near-certain low-score result", () => {
  const target: MatchRow = { date: "2026-09-15", home: "Wadi Degla", away: "Al Mokawloon", league: "Egyptian Premier League" };
  const globalHistory: MatchRow[] = Array.from({ length: 40 }, (_, i) => ({ date: `2026-01-${String((i % 28) + 1).padStart(2, "0")}`, home: i % 2 ? "Team A" : "Team B", away: i % 2 ? "Team C" : "Team D", hg: i % 4 === 0 ? 0 : i % 3 === 0 ? 2 : 1, ag: i % 5 === 0 ? 2 : i % 2, result: "H" as const, league: "Egyptian Premier League" }));
  ...
  assert.ok(result.confidence < 70, `Sparse evidence confidence must remain limited; got ${result.confidence}`);
  assert.ok(result.quality < 70, `Sparse evidence quality must remain limited; got ${result.quality}`);
...
test("audit regression: event probability and epistemic confidence are distinct", () => {
  ...
  assert.equal(result.aiReasoningPacket.confidenceDefinition, "Epistemic confidence is evidence/coverage/consensus quality; it is never equal to event probability.");
});