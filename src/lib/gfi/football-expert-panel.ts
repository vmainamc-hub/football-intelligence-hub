...
const emptyPanel = (status: FootballExpertPanel["status"], model: string, note: string, analysis?: AuthoritativeMatchAnalysis): FootballExpertPanel => {
  ...
  return {
    ...
    analystCall: {
      status: "FALLBACK",
      market: fallback?.market ?? "QUANTITATIVE",
      selection: fallback?.label ?? analysis?.finalPrediction ?? "Quantitative engine result",
      callType: "OTHER",
      rationale: "The AI council was unavailable, so the validated quantitative selection was retained.",
      conviction: "LOW",
      evidenceQuality: "LIMITED",
      quantitativeLeader: fallback?.label ?? analysis?.finalPrediction ?? "Not supplied",
      quantitativeProbability: Math.round((fallback?.modelProbability ?? 0) * 100),
...
function marketSurface(analysis: AuthoritativeMatchAnalysis): Array<{ market: string; selection: string; probability: number; qualification: string }> {
  ...
  const rows = candidates
    ...
    .map((c: MarketCandidate) => ({ market: c.market, selection: c.selection, probability: Math.round(c.modelProbability * 100), qualification: c.qualificationStatus }));
...
function cleanPanel(value: any, model: string, analysis: AuthoritativeMatchAnalysis): FootballExpertPanel {
  ...
  return {
    ...
    teamStrength: {
      home: { relative: validRelative(raw.teamStrength?.home?.relative), rationale: text(raw.teamStrength?.home?.rationale, "No supported assessment supplied.") },
      away: { relative: validRelative(raw.teamStrength?.away?.relative), rationale: text(raw.teamStrength?.away?.rationale, "No supported assessment supplied.") },
      strengthGap: ["HOME_CLEAR", "AWAY_CLEAR", "CLOSE", "UNKNOWN"].includes(raw.teamStrength?.strengthGap) ? raw.teamStrength.strengthGap : "UNKNOWN",
      opponentQualityAdjustment: text(raw.teamStrength?.opponentQualityAdjustment, "Opponent-quality adjustment not established from supplied evidence."),
    },
    ...
    analystCall: {
      status: "ACTIVE",
      market: selectedRow?.market ?? fallbackRow?.market ?? "QUANTITATIVE",
      selection: finalSelection,
      callType: callType as AIAnalystCallType | "OTHER",
      rationale: text(raw.analystCall?.rationale, "The council selected the strongest supported market from the computed market surface."),
      conviction,
      evidenceQuality,
      quantitativeLeader,
      quantitativeProbability: finalRow?.probability ?? 0,
      divergenceFromQuantitativeLeader: divergence,
      divergenceReason: divergence ? text(raw.analystCall?.divergenceReason, "The council judged the quantitative leader less informative than the selected football conclusion.") : "",
      secondaryCall: text(raw.analystCall?.secondaryCall) || undefined,
      guardrails: safeGuardrails.filter((x: unknown): x is string => typeof x === "string").slice(0, 8),
    },
...
Council roles (all must contribute): 1) Team Strength Scout — true relative strength, opponent quality, cross-competition distortion, attack/defence level, venue strength. 2) Tactical Analyst — style matchup, pressing, transitions, defensive structure, game-state implications. 3) Statistical Analyst — interrogates the quantitative probabilities, calibration, score distribution and model agreement. 4) Form & Trajectory Analyst — separates sustainable performance from noisy recent results and schedule effects. 5) Context & Motivation Analyst — only uses supplied current context; never invents motivation. 6) Competition Strength Analyst — checks whether competition labels and team context are coherent and whether cross-league priors could distort the result. 7) Data Forensic Analyst — hunts identity mismatch, contamination, duplicate evidence, sparse samples and model/data contradictions. 8) Contrarian Analyst — constructs the strongest credible counter-case to the emerging conclusion.

Process: FIRST verify fixture identity and competition. SECOND assess team strength before interpreting raw form. THIRD interrogate the model surface. FOURTH debate the strongest disagreement. FIFTH Chair decides the most informative football conclusion, not automatically the safest market. A high-probability safe market may be rejected in favor of a directional outcome when the council has a defensible football reason. Conversely, do not force a winner when the evidence genuinely supports goals/BTTS instead. The final call must be exactly one selection from marketSurface. Never use an invented probability for the AI call. Conviction is qualitative and means strength of analytical evidence, not event probability.

Guardrails: identity FAIL or severe data contamination => REVALIDATE and retain quantitative result as fallback. Identity PASS/WARN with coherent evidence => council may select any supported computed market. Limited evidence does not automatically force PASS/NO_TRADE; label evidence quality honestly. Do not let the council simply majority-vote: the Chair weighs evidence quality, specialist expertise, contradiction severity and football plausibility. Return JSON only. Include exactly eight panel experts, a debate, Chair synthesis, and analystCall.`;
  ...
  try {
    ...
    return cleanPanel(parsed, model, analysis);
  } catch (error) {
