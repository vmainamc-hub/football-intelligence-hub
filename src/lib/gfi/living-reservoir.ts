...
async function refreshCell(db: SupabaseClient, fixture: MatchRow) {
  ...
  const researched = await researchTeamOrFixture(`${home} vs ${away} ${dateKey(fixture.date)}`).catch(() => undefined);
  const evidenceCount = researched?.matches.length ?? 0;
  const sources = researched?.sources ?? [];
  const completeness = Math.min(100, Math.round((researched?.coverage ?? 0) * 0.7 + Math.min(30, sources.length * 6)));
  ...
  await writeObservation(db, "research-orchestrator", "living-match-research", "match", key, researched ?? { matches: [] }, Math.max(0.5, completeness / 100));

  for (const [teamKey, teamName] of [[homeKey, home], [awayKey, away]] as const) {
    const history = (researched?.matches ?? []).filter((m) => canonicalTeamKey(m.home) === teamKey || canonicalTeamKey(m.away) === teamKey);
    await db.from("team_intelligence_cells").upsert({
      ...
      source_count: sources.length,
      completeness: Math.min(100, Math.round((researched?.coverage ?? 0) * 0.8 + Math.min(20, sources.length * 5))),
      status: "ACTIVE",
      summary: {
        team: teamName,
    ...
    await writeObservation(db, "research-orchestrator", "living-team-research", "team", teamKey, { team: teamName, matches: history, sources }, Math.max(0.5, (researched?.coverage ?? 0) / 100));
  }
...
export async function mineLivingReservoir(fixtures: MatchRow[], budget = 20) {
  const db = admin();
  if (!db || !fixtures.length) return { configured: Boolean(db), requested: fixtures.length, processed: 0, evidence: 0, sources: 0 };
  const now = Date.now();
  ...
  const run = await db.from("evidence_mining_runs").insert({ run_type: "LIVING_FIXTURE_WARM", requested_count: selected.length, status: "RUNNING", detail: {} }).select("id").maybeSingle();
  let processed = 0, evidence = 0, sources = 0;
  ...
  if (run.data?.id) {
    await db.from("evidence_mining_runs").update({ processed_count: processed, evidence_found: evidence, sources_with_data: sources, finished_at: new Date().toISOString(), status: "SUCCESS", detail: { durationMs: Date.now() - now } }).eq("id", run.data.id);
  }
...
export const livingReservoirStats = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const latest = await db.from("match_intelligence_cells").select("last_mined_at").order("last_mined_at", { ascending: false }).limit(1).maybeSingle();
  return { configured: true, teams: teams.count ?? 0, matches: matches.count ?? 0, observations: observations.count ?? 0, lastMinedAt: latest.data?.last_mined_at ?? null };
});
