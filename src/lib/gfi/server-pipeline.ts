...
export function analyzeLoadedFixture(fixture: MatchRow, code: string, groups: FreeLeague[]): ServerMatchAnalysis {
  const group = groups.find((item) => item.code === code) ?? groups.find((item) => item.league === fixture.league);
  const targetRows = prepareHistoricalMatches(fixture, group?.matches ?? []);
  const globalRows = prepareHistoricalMatches(fixture, groups.flatMap((item) => item.matches));
  const historical = dedupeRows([...targetRows, ...globalRows]);
  const homeRows = historical.filter((row) => sameTeam(row.home, fixture.home) || sameTeam(row.away, fixture.home));
  const awayRows = historical.filter((row) => sameTeam(row.home, fixture.away) || sameTeam(row.away, fixture.away));
  ...
  const result = analyzeAuthoritatively({ ...fixture }, historical);
  ...
  return { ...result, engines, decision: finalDecision, finalPrediction, pipeline: {
    fixtureId: fixtureId(fixture), source: fixture.source ?? "free-data", competition: fixture.league ?? group?.league ?? "Worldwide Football", competitionsLoaded: groups.length,
    historicalRowsLoaded: historical.length, targetCompetitionRows: targetRows.length, homeRowsMatched: homeRows.length, awayRowsMatched: awayRows.length,
    homeSample: result.home.played, awaySample: result.away.played, h2hSample: result.engines.find((engine) => engine.id === "H2H")?.values.matches ?? 0,
    enginesRun: engines.map((engine) => engine.id), evidenceMode,
    simulation: { iterations: simulation.summary.iterations, homeWin: simulation.summary.homeWin, draw: simulation.summary.draw, awayWin: simulation.summary.awayWin, agreement: simulation.summary.scenarioAgreement }, generatedAt
  }};
}
