...
const goal = (r: AuthoritativeMatchAnalysis, k: string) =>
  r.engines.find((e) => e.id === "TOTALS")?.values?.[k];
...
function signal(
  ...
  const p = clamp(probability),
    c = clamp(result.confidence / 100) * 100;
  return {
    market,
    selection,
    probability: p,
...
export function buildSevenMarketComparison(result: AuthoritativeMatchAnalysis): MarketSignal[] {
  const p = result.probabilities,
    h = result.home.team,
    a = result.away.team,
    o15 = Number(goal(result, "over1.5")),
    o25 = Number(goal(result, "over2.5")),
    o35 = Number(goal(result, "over3.5")),
    b = Number(btts(result));
  return [
    ...
    signal(
      ...
      1 - o35,
      result,
      `Under 3.5 probability ${pct(1 - o35)}%.`,
    ),
...
export function buildMainstreamMarketMap(result: AuthoritativeMatchAnalysis): MarketSignal[] {
  ...
  const { home, draw, away } = result.probabilities,
    ht = result.home.team,
    at = result.away.team,
    winner = home >= draw && home >= away ? ht : away >= draw ? at : "DRAW";
  const out: MarketSignal[] = [
    signal(
      "1X2",
      winner === "DRAW" ? "DRAW" : `${winner} WIN`,
      Math.max(home, draw, away),
      result,
      `Highest 1X2 probability ${pct(Math.max(home, draw, away))}%.`,
    ),
  ];
  const dc = [
  ...
  out.push(
    signal(
      ...
      dc.probability,
      result,
      `Two-result coverage ${pct(dc.probability)}%.`,
    ),
  );
  const dnbH = home / Math.max(0.0001, home + away),
    dnbA = away / Math.max(0.0001, home + away);
  out.push(
    signal(
      "DRAW NO BET",
      dnbH >= dnbA ? `DNB — ${ht}` : `DNB — ${at}`,
      Math.max(dnbH, dnbA),
      result,
      "Draw removed from the win comparison.",
    ),
  );
  for (const line of [1.5, 2.5, 3.5] as const) {
    const o = Number(goal(result, `over${line}`));
    if (!Number.isFinite(o)) continue;
    out.push(
      signal(
        `OVER/UNDER ${line}`,
        o >= 0.5 ? `OVER ${line}` : `UNDER ${line}`,
        Math.max(o, 1 - o),
        result,
        `Goal model probability ${pct(Math.max(o, 1 - o))}%.`,
      ),
    );
  }
  ...
  if (Number.isFinite(b))
    out.push(
      signal(
        "BTTS",
        b >= 0.5 ? "BTTS — YES" : "BTTS — NO",
        Math.max(b, 1 - b),
        result,
        `BTTS selection probability ${pct(Math.max(b, 1 - b))}%.`,
      ),
    );
  return out;
}
export function bestQualifiedMarket(result: AuthoritativeMatchAnalysis): MarketSignal {
  const markets = buildMainstreamMarketMap(result),
    insufficient =
      ...
      Number(result.aiReasoningPacket?.teamSample ?? 0) < 8 ||
      result.aiReasoningPacket?.sparsePriorUsed === true;
  ...
  const quality = result.quality,
    ...
    robust = result.robustness.score,
    risk = result.risk;
  const ranked = markets.slice().sort((a, b) => {
    const score = (m: MarketSignal) =>
      ...
      (robust / 100) * 0.15 +
      (quality / 100) * 0.2 -
      (risk === "HIGH" ? 0.08 : risk === "VERY HIGH" ? 0.15 : 0);
    return score(b) - score(a);
  });
  const best = ranked[0],
    qualified =
      !insufficient && best.confidence >= 60 && best.probability >= 0.57 && risk !== "VERY HIGH";
  return {
    ...best,
    tier: qualified ? (best.probability >= 0.62 ? "PRIMARY" : "SECONDARY") : "WATCH",
    rationale: qualified
      ? best.rationale
      : `${best.rationale} Strongest mathematical market, but not qualified for action because direct evidence is insufficient or epistemic confidence is ${best.confidence}% (risk ${risk}).`,
  };
}
