...
const tier = (p: number, c: number): MarketSignal["tier"] => c < 50 ? "WATCH" : p >= .62 ? "PRIMARY" : p >= .56 ? "SECONDARY" : "WATCH";
...
export function buildSevenMarketComparison(result: AuthoritativeMatchAnalysis): MarketSignal[] {
  const p = result.probabilities, h = result.home.team, a = result.away.team;
  const o15 = Number(goal(result, "over1.5")), o25 = Number(goal(result, "over2.5")), o35 = Number(goal(result, "over3.5")), b = Number(btts(result));
  return [
    ...
    signal("OVER/UNDER 1.5", "OVER 1.5", o15, result, `Over 1.5 probability ${pct(o15)}%.`),
    signal("OVER/UNDER 2.5", "OVER 2.5", o25, result, `Over 2.5 probability ${pct(o25)}%.`),
    signal("OVER/UNDER 3.5", "UNDER 3.5", 1 - o35, result, `Under 3.5 probability ${pct(1 - o35)}%.`),
...
export function buildMainstreamMarketMap(result: AuthoritativeMatchAnalysis): MarketSignal[] {
  ...
  const ht = result.home.team, at = result.away.team;
  const winner = home >= draw && home >= away ? ht : away >= draw ? at : "DRAW";
  ...
  const dnbH = home / Math.max(.0001, home + away), dnbA = away / Math.max(.0001, home + away);
  out.push(signal("DRAW NO BET", dnbH >= dnbA ? `DNB — ${ht}` : `DNB — ${at}`, Math.max(dnbH, dnbA), result, "Draw removed from the win comparison."));
  for (const line of [1.5, 2.5, 3.5] as const) { const o = Number(goal(result, `over${line}`)); if (!Number.isFinite(o)) continue; out.push(signal(`OVER/UNDER ${line}`, o >= .5 ? `OVER ${line}` : `UNDER ${line}`, Math.max(o, 1 - o), result, `Goal model probability ${pct(Math.max(o, 1 - o))}%.`)); }
  const b = Number(btts(result));
  if (Number.isFinite(b)) out.push(signal("BTTS", b >= .5 ? "BTTS — YES" : "BTTS — NO", Math.max(b, 1 - b), result, `BTTS selection probability ${pct(Math.max(b, 1 - b))}%.`));
  return out;
}
...
export function bestQualifiedMarket(result: AuthoritativeMatchAnalysis): MarketSignal {
  ...
  const insufficient = result.decision === "INSUFFICIENT INTELLIGENCE" || Number(result.aiReasoningPacket?.teamSample ?? 0) < 8 || result.aiReasoningPacket?.sparsePriorUsed === true;
  if (!markets.length) return { market: "1X2", selection: "NO MARKET DATA", probability: 0, confidence: 0, tier: "WATCH", rationale: "No market observations were produced." };
  ...
  const ranked = markets.slice().sort((a, b) => {
    const score = (m: MarketSignal) => m.probability * .40 + (m.confidence / 100) * .25 + (robust / 100) * .15 + (quality / 100) * .20 - (risk === "HIGH" ? .08 : risk === "VERY HIGH" ? .15 : 0);
    return score(b) - score(a);
  });
  const best = ranked[0];
  const qualified = !insufficient && best.confidence >= 60 && best.probability >= .57 && risk !== "VERY HIGH";
  return { ...best, tier: qualified ? (best.probability >= .62 ? "PRIMARY" : "SECONDARY") : "WATCH", rationale: qualified ? best.rationale : `${best.rationale} Strongest mathematical market, but not qualified for action because direct evidence is insufficient or epistemic confidence is ${best.confidence}% (risk ${risk}).` };
}

export function primaryMarket(result: AuthoritativeMatchAnalysis) {
  return buildMainstreamMarketMap(result)[0] ?? { market: "1X2", selection: "NO MARKET DATA", probability: 0, confidence: 0, tier: "WATCH" as const, rationale: "No market data." };
}