...
function h2hRows(home: string, away: string, matches: ReservoirMatch[]) {
  ...
  return matches
    .filter((m) => canonicalTeamKey(m.home) === hk && canonicalTeamKey(m.away) === ak || canonicalTeamKey(m.home) === ak && canonicalTeamKey(m.away) === hk)
...
async function mineMatch(db: SupabaseClient, fixture: MatchRow) {
  ...
  const [homeHistory, awayHistory, research] = await Promise.all([
  ...
  const sourceFamilies = [...new Set([
    ...history.map((r) => r.source ?? "reservoir"),
    ...(research?.sources ?? []),
  ])];
  const evidenceCount = history.length + (research?.matches.length ?? 0) + h2h.length;
  const completeness = Math.min(100, Math.round(
    Math.min(40, history.length / 10) +
    Math.min(20, h2h.length * 2) +
    Math.min(20, sourceFamilies.length * 4) +
    Math.min(20, (research?.coverage ?? 0) * 0.2),
  ));
  ...
  await writeObservation(db, "deep-miner", "deep-match-context", "match", matchKey, {
    fixture: { ...fixture, home, away }, home: homeAll, away: awayAll,
    homeVenue: homeHome, awayVenue: awayAway, h2h: h2h.slice(0, 20), sources: sourceFamilies,
...
async function mineTeam(db: SupabaseClient, teamName: string) {
  ...
  await db.from("team_intelligence_cells").upsert({
    ...
    source_count: [...new Set(history.map((m) => m.source ?? "reservoir"))].length,
    completeness,
    status: "ACTIVE",
    summary: { team: name, all, home, away, competitions, minedAt: now },
    updated_at: now,
  }, { onConflict: "team_key" });
  await writeObservation(db, "deep-miner", "deep-team-context", "team", teamKey, { team: name, all, home, away, competitions }, Math.max(0.55, completeness / 100));
