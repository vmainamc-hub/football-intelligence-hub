...
export const harvestHistoricalSeasons = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const seasons = [
      ...new Set((data.seasons ?? ["2627", "2526", "2425"]).filter((x) => /^\d{4}$/.test(x))),
    ].slice(0, 3);
    const totals = {
      requested: 0,
      successful: 0,
    ...
    for (const season of seasons) {
      for (let i = 0; i < CODES.length; i += 4) {
        ...
        const settled = await Promise.all(
          batch.map(([code, league]) => fetchSeason(code, league, season)),
        );
        for (let j = 0; j < settled.length; j += 1) {
          ...
          if (outcome.rows.length) {
            ...
            totals.failed += 1;
            errors.push(`${season}/${code} ${league}: ${outcome.error ?? "unknown"}`);
          }
        }
      }
    }
    ...
    totals.competitions = new Set(
      allRows.map((r) => `${r.code}|${r.season ?? ""}|${r.league}`),
    ).size;
    if (allRows.length)
      await ingestReservoirMatches(allRows, `football-data-historical-${seasons.join("-")}`);
