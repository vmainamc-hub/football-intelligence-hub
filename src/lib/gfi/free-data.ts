...
async function fetchFreeLeagueCsvInternal(codeValue: string, season: string): Promise<FreeDataFetchResult> {
  ...
  if (!validCode(code) || !validSeason(season)) throw new Error("Invalid free football data request");
  ...
  try {
    ...
    return { code, season, csv, sourceUrl, fetchedAt: new Date().toISOString() };
  } finally {
    clearTimeout(timer);
  }
}

export { fetchFreeLeagueCsvInternal };

export const fetchFreeLeagueCsv = createServerFn({ method: "GET" })
  .validator((input: { code: string; season: string }) => input)
  .handler(async ({ data }): Promise<FreeDataFetchResult> => fetchFreeLeagueCsvInternal(data.code, data.season));
